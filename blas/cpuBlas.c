...
int main(int argc, char **argv) {
  ...
  #ifdef _OPENMP
    double training_start = omp_get_wtime();
  #else
  ...
  for (int k = 0; k < num_epochs; k++) {
    ...
    double training_time = (double)(t2 - training_start);
  #else
    clock_t t2 = clock();
    double training_time = (double)(t2 - training_start) / CLOCKS_PER_SEC;
  #endif
  ...
  #ifdef _OPENMP
    t2 = omp_get_wtime();
    double inference_time = (double)(t2 - inference_start);
  #else
    t2 = clock();
    double inference_time = (double)(t2 - inference_start) / CLOCKS_PER_SEC;
  #endif
