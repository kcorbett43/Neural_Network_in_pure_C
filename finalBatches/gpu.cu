...
void gpuMatMul(float *output, float *data1, float *data2, int m, int n, int p) {
...
void matMul(float *output, float *data1, float *data2, int m, int n, int p) {
  //naiveMatMul(output, data1, data2, m, n, p);
  //tileMatMul(output, data1, data2, m, n, p);
  gpuMatMul(output, data1, data2, m, n, p);
}
...
void linearLayer(float *output, float *input, float *weights, float *bias, int m, int n, int p) {
  #ifdef HAVE_OPENBLAS
  ...
  #else
    matMul(output, input, weights, m, n, p); 
...
void gpuMatMulTranspose(float *output, float *transpose, float *mat, int m, int n, int p){
  // Allocate memory on device
  float *dev_output;
  cudaMalloc((void **)&dev_output, m*p*sizeof(float));
  float *dev_transpose;
...
void matMulTranspose(float *output, float *transpose, float *mat, int m, int n, int p){
  //tileMatMulTranspose(output, transpose, mat, m, n, p);
  gpuMatMulTranspose(output, transpose, mat, m, n, p);
}
...
void gpuMatMulTransposeOpp(float *output, float *transpose, float *mat, int m, int n, int p){
  // Allocate memory on device
  float *dev_output;
  cudaMalloc((void **)&dev_output, m*p*sizeof(float));
  float *dev_transpose;
...
// calcs A * B^T
void matMulTransposeOpp(float *output, float *mat, float *transpose, int m, int n, int p){
  //naiveMatMulTransposeOpp(output, mat, transpose, m, n, p);
  //tileMatMulTransposeOpp(output, mat, transpose, m, n, p);
  gpuMatMulTransposeOpp(output, transpose, mat, m, n, p);
}
...
void hiddenBackward(float *acc_dW, float *acc_db, float *next_W, float *next_del, float *a, int batch_size, int cols, int w_rows, float *prev_a, float *del_WL, int prev_w_len) {
  ...
  #else
    matMulTransposeOpp(del_WL, next_del, next_W, batch_size, w_rows, cols);
  #endif
  ...
  #else
    matMulTranspose(grad_W, prev_a, del_WL, prev_w_len, batch_size, cols);
...
void softmaxBackward(float *acc_dW, float *acc_db, float *del_L, float *a_l, float *a_l_1, int *batch_labels, int batch_size, int cols, int w_rows) { 
  ...
  #else
    matMulTranspose(del_WL, a_l_1, del_L, w_rows, batch_size, cols); // 7
...
int main(int argc, char **argv) {
  ...
  for (int k = 0; k < num_epochs; k++) {
    ...
    for (int i = 0; i < num_batches-1; i++) { 
      ...
      linearLayer(a1, batch_inputs, W_1, b_1, batch_size, img_len, num_layer1_neurons);
      ReLU(a1, batch_size, num_layer1_neurons);
      linearLayer(a2, a1, W_2, b_2, batch_size, num_layer1_neurons, num_layer2_neurons);
      ReLU(a2, batch_size, num_layer2_neurons);
      linearLayer(a3, a2, W_3, b_3, batch_size, num_layer2_neurons, num_outputs);
      crossEntropyLoss(a3, batch_labels, batch_size);
      softmaxBackward(acc_dW_3, acc_db_3, del_L, a3, a2, batch_labels, batch_size, num_outputs, num_layer2_neurons);
      hiddenBackward(acc_dW_2, acc_db_2, W_3, del_L, a2, batch_size, num_layer2_neurons, num_outputs, a1, del_l2, num_layer1_neurons);
      hiddenBackward(acc_dW_1, acc_db_1, W_2, del_l2, a1, batch_size, num_layer1_neurons, num_layer2_neurons, batch_inputs, del_l1, img_len);
    ...
    linearLayer(a1v, val_inputs, W_1, b_1, num_validation, img_len, num_layer1_neurons);
    ReLU(a1v, num_validation, num_layer1_neurons);
    linearLayer(a2v, a1v, W_2, b_2, num_validation, num_layer1_neurons, num_layer2_neurons);
    ReLU(a2v, num_validation, num_layer2_neurons);
    linearLayer(a3v, a2v, W_3, b_3, num_validation, num_layer2_neurons, num_outputs);
  ...
  for (int i = 0; i<num_test_examples; i++) {
    ...
    linearLayer(a1, test_data[i], W_1, b_1, 1, img_len, num_layer1_neurons);
    ReLU(a1, 1, num_layer1_neurons);
    linearLayer(a2, a1, W_2, b_2, 1, num_layer1_neurons, num_layer2_neurons);
    ReLU(a2, 1, num_layer2_neurons);
    linearLayer(a3, a2, W_3, b_3, 1, num_layer2_neurons, num_outputs);
